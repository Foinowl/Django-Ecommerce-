...
class ViewPage(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(ViewPage, self).get_context_data(**kwargs)
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    ...
                    messages.warning(self.request, "Invalid selected option")
                    return redirect('checkout')
        except ObjectDoesNotExist:
...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context[ 'cartItems' ] = 0
        myFilter = ItemsFilter(self.request.GET, Item.objects.all())
...
class ItemDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context[ 'cartItems' ] = 0
        p = Item.objects.get(slug=self.kwargs.get('slug'))
        context['other'] =Item.objects.filter(category=p.category).exclude(pk=p.pk)[:3]
        return context

class ContentDetailView(ListView):
    model = Item
    paginate_by = 4
    template_name = 'store/content_items.html'
    ...
    def get_queryset(self):
        ...
        if category is not None:
            print(category)
            return ItemsFilter(self.request.GET, queryset=Item.objects.filter(category=category)).qs
        return ItemsFilter(self.request.GET, queryset=Item.objects.all()).qs
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context[ 'cartItems' ] = 0
        myFilter = ItemsFilter(self.request.GET, queryset=self.get_queryset())
        context[ 'filter' ] = myFilter
        return context
